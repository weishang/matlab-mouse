...
% Last Modified by GUIDE v2.5 15-Apr-2012 01:16:56
...
bounds = get(handles.lbl_target_1,'position');
lx = bounds(1); ly = bounds(2);
lw = bounds(3); lh = bounds(4);
% test to see if mouse is within the uicontrol. 
if x >= lx && x <= (lx + lw) && y >= ly && y <= (ly + lh)
    set(handles.lbl_target_1, 'string', 'IN');
    set(handles.lbl_target_1, 'backgroundcolor', 'red');
    
    % change to hand shape pointer     
    setfigptr('hand', handles.fig_mouse);
else
    set(handles.lbl_target_1,'string', 'OUT');
    set(handles.lbl_target_1, 'backgroundcolor', 'green');    
